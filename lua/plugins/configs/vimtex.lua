...
function getos()
  ...
  handle:close()
  osname = osname:gsub("%s+", "")
  return osname
end

if getos() == "Linux" then
  print("Found system Linux")
  vim.g.vimtex_view_general_viewer = "okular"
  vim.g.vimtex_view_general_options = "--unique file:@pdf#src:@line@tex"
elseif getos() == "Darwin" then
  print("Found system MacOS")
  ...
  print("Found unexpected OS" .. osname)
end
...
-- skip latex environment when using gqip
vim.cmd([[
fun! TeX_fmt()
    if (getline(".") != "")
    let save_cursor = getpos(".")
        ...
        set nowrapscan
        let par_begin = '^\(%D\)\=\s*\($\|\\begin\|\\end\|\\Begin\|\\End\|\\\(sub\)*section\>\|\\item\>\|\\NC\>\|\\blank\>\|\\noindent\>\)'
        " let par_begin = '^\(%D\)\=\s*\($\|\\begin\|\\end\|\\Begin\|\\End\|\\\(sub\)*section\>\|\\item\>\|\\NC\>\|\\blank\>\|\\noindent\>\)'
        let par_end   = '^\(%D\)\=\s*\($\|\\begin\|\\end\|\\Begin\|\\End\|\\place\|\\\(sub\)*section\>\|\\item\>\|\\NC\>\|\\blank\>\)'
        " let par_end   = '^\(%D\)\=\s*\($\|\\begin\|\\end\|\\Begin\|\\End\|\\place\|\\\(sub\)*section\>\|\\item\>\|\\NC\>\|\\blank\>\|\s\|\\\\\>\)'
    try
      exe '?'.par_begin.'?+'
    catch /E384/
      1
